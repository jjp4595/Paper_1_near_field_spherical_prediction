...
Considering medium to large scaled distances specifically, the KB charts have proven to be accurate \cite{rigbyValidationSemiempiricalBlast2014, karlosAnalysisBlastParameters2016} and recent work developing blast wave decay coefficients from the KB data are proposed in \cite{karlosAnalysisBlastWave2016}.
...
Alternate blast load models based on the RPB model are proposed in \cite{jangBlastLoadModel2018,dharmasenaMechanicalResponseMetallic2008}, though the authors do not perform a direct comparison of the loading itself and instead compare them through analysing plate responses to the loading model as a proxy.

Alternative approaches by Pannell et al \cite{pannellPredictingNearFieldSpecific2019} involve modelling specific impulse distributions using two superposed Gaussian functions and perform non-linear regressions to build a parameter space for a given dataset of CFD results which shows promise. 
Work by Rigby et al \cite{rigbyPredictingResponsePlates2019} propose the concept of ``upper bound kinetic energy uptake'' and demonstrate that in order to obtain any meaningful prediction of plate response, both the magnitude and distribution of the loading must be known.
...
Whilst acknowledging that overpressure is interesting from an academic perspective, this research instead focuses only on specific impulse distributions for PE4 spheres in the extreme near-field due to its direct representation of the loading.
...
\begin{figure}[htpb]
	...
	\caption{Contour plot peak impulse distribution of 100g PE4 according to the RPB model with MCEER values. TNT equivalence of PE4 assumed as 1.2.}
	\label{fig:theta_peak_impulse_RPB_MCEER_plate}
\end{figure}
\subsection{Blast Impact Impulse Model \cite{remennikovExperimentalInvestigationSimplified2017}}
Analytical approach based on instantaneous detonation of spherical charge.
