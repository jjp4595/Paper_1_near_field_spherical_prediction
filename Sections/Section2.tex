...
Common near-field experimental measurement techniques consist of measuring momentum imparted to small, rigid plugs embedded within a larger target surface \cite{huffingtonReflectedImpulseSpherical1985, nansteelImpulsePlugMeasurements2013} and the use of Hopkinson pressure bars (HPBs) 
\cite{hopkinsonMethodMeasuringPressure1914,kolskyInvestigationMechanicalProperties1949, edwardsBlastWaveMeasurements1992,piehlerNearFieldImpulseLoading2009,rigbyTestingApparatusSpatial2014,rigbyObservationsPreliminaryExperiments2015,tyasExperimentalStudiesEffect2016, cloeteBlastCharacterizationUsing2016, rigbyNearfieldBlastLoading2019}.
...
\subsection{Loading distribution models}
\subsubsection{Randolph-Pehrson Bannister model with MCEER data}
...
\end{figure}
\subsubsection{Henrych / Remennikov analytical model}
From \textcite{henrychDynamicsExplosionIts1979} and Remennikov \cite{remennikovExplosiveBreachingWalls2015, remennikovBIIMModelParameters2020}, we have a theoretical basis to calculate specific impulse distributions. 
The hypothesis of a constant velocity for out flowing particles is introduced:
\begin{equation}
u = u_x = \text{ constant}.
\end{equation}
Given the explosive parameters: standoff, $a$(m); charge radius, $r_0$(m); detonation velocity, $D_0$(m/s); explosive energy, $Q_0$(MJ/kg) and density, $\rho_0$(kg/$m^3$).

Detonation pressure (Pa) is given by:
\begin{equation}
	P_0 = \frac{1}{2}\rho_0D_0^2
\end{equation}
Detonation products velocity (m/s):
\begin{equation}
	u_0 = \sqrt{Q_0 \times 2 \times 10^6}
\end{equation}
Velocity of hypothetical surface (m/s):
\begin{equation}
	w_0 = P_0 \div \rho_0 \div u_0
\end{equation}
Leading to the explosive constant (m/s):
\begin{equation}
	A_0 = \frac{u_0 + w_0}{4\pi}
\end{equation}
Finally, specific impulse ($kPa.ms$):
\begin{equation}
i = \left(\frac{A_0W}{a^2}\right)cos^4\theta,
\label{eq:Henrych_sphere}
\end{equation}
where $\theta$ is the angle of incidence as given in Figure \ref{fig:RPB}.