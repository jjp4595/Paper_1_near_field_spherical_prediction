...
\begin{figure}[htpb]
	...
	\begin{subfigure}[htpb]{0.45\textwidth}
		...
		\caption{$0.2 {\footnotemark} m/kg^{1/3} \leq z \leq 0.55 m/kg^{1/3}$}
		\label{fig:data_overview_large_b}
	\end{subfigure}	
	\begin{subfigure}[htpb]{0.45\textwidth}
		\centering
		\includegraphics{Graphs/data_overview_large_c.pdf}
		\caption{$0.1m/kg^{1/3} \leq z \leq 0.55 m/kg^{1/3}$}
		\label{fig:data_overview_large_c}
	\end{subfigure}
	\caption{Peak specific impulse distribution overview of PE4 datasets $0.1m/kg^{1/3} \leq z \leq 0.55 m/kg^{1/3}$. }
\footnotetext{Note: in this figure data for close-in scaled distances have been removed for clarity.}
\end{figure}
...
\begin{figure}[htpb]
	...
	\begin{subfigure}[htpb]{\textwidth}
		...
		\vspace{2mm}
		\caption{$0.10m/kg^{1/3} \leq z \leq 0.55 m/kg^{1/3}$}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Gaussian model for theta distribution for $0.1 \leq z \leq 0.55m/kg^{\frac{1}{3}}$.}
\end{figure}

\subsection{Proposed model summary}
Two models have been proposed, the first a more accurate model across a smaller range of scaled distance whilst the latter provides a moderate level of accuracy across a wide range of scaled distances.
Summarised below:

For $0.10 \leq z \leq 0.20m/kg^{\frac{1}{3}}$ and $0^{\circ} \leq \theta \leq 80^{\circ}$:
\begin{equation}
i(z, \theta, W) = W^{\frac{1}{3}} \times 0.383z^{-1.858} \times
exp \left( \frac{-\left( \frac{\theta}{160} \right) ^2}{2 \times 0.189^2} \right).
\end{equation}

And for $0.10 \leq z \leq 0.55m/kg^{\frac{1}{3}}$ and $0^{\circ} \leq \theta \leq 80^{\circ}$:
\begin{equation}
i(z, \theta, W) =  W^{\frac{1}{3}} \times 0.557z^{-1.663} \times
exp \left( \frac{-\left( \frac{\theta}{160} \right) ^2}{2 \times 0.198^2} \right).
\end{equation}

Where $z$ is scaled distance, $W$ is TNT equivalent charge mass and $\theta$ is angle of incidence.
