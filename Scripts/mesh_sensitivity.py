...
ax.plot(0.08*np.tan(np.deg2rad(theta))*1e3, savgol_filter(z80mm_chosenmesh_adjustedi/1e3,101,3), 'r', ls = '-', lw = 0.5, label = 'CFD')
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles, labels, loc='upper right', prop={'size':6})
ax.set_ylim(0,5)
ax.set_xlim(0,100)
ax.set_xlabel('radial ordinate from centre (m)')
ax.set_ylabel('peak specific impulse (MPa.ms)')
ax.minorticks_on()
ax.grid(which='minor', alpha=0.2)
ax.grid(which='major', alpha=0.5) 
...
ax.plot(gtable380mm[0][:,2]*1e3,gtable380mm[0][:,7]/1e3, 'r', ls = '-', lw = 0.5, label = 'CFD')
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles, labels, loc='upper right', prop={'size':6})
ax.set_ylim(0,0.5)
ax.set_xlim(0,100)
