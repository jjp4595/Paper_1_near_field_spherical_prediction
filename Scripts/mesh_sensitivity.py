...
# ax.scatter(theta_exp_80mm, np.divide(MxI_3_80mm, max(MxI_3_80mm)), marker="x", s=15., color=[0.75,0.75,0.75], edgecolors='none')
# ax.scatter(theta_exp_80mm_mean, np.divide(Mx_mean_80mm, max(Mx_mean_80mm)), marker="o", s=15., label = '80mm Exp Mean')
# ax.plot(theta, savgol_filter(z80mm_chosenmesh_adjustedi/1e3,101,3)/max(savgol_filter(z80mm_chosenmesh_adjustedi/1e3,101,3)), 'r', ls = '-', lw = 0.5, label = 'CFD')
# handles, labels = ax.get_legend_handles_labels()
# ax.legend(handles, labels, loc='upper right', prop={'size':6})
# ax.set_ylim(0,1)
# ax.set_xlim(0,80)
...
# ax.grid(which='major', alpha=0.5) 
# plt.tight_layout()
# fig2.savefig(os.environ['USERPROFILE'] + r'\Dropbox\Papers\Paper_1_near_field_spherical_prediction\Graphs\mesh_convergence_z0_055_3a.pdf', format = 'pdf')
#-----------------------------------------------------------------------------
